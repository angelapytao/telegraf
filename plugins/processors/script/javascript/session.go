...
func newSession(p *goja.Program, conf *Config, test bool) (*session, error) {
	...
	s.vm.Set("Event", newMetircEventConstructor(s))
...
func (s *session) setEvent(b telegraf.Metric) error {
	if s.evt == nil {
		var err error
...
func (s *session) runProcessFunc(b telegraf.Metric) (out telegraf.Metric, err error) {
	defer func() {
		if r := recover(); r != nil {
			s.log.Errorf("The javascript processor caused an unexpected panic "+
				"while processing an event. Recovering, but please report this.",
				"event", common.MapStr{"original": b.Fields},
				"panic", r,
				zap.Stack("stack"))
